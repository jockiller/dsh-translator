/* dsh-plugin-translator client bundle - v0.0.9 */
(function() {
...
return {
  ...
  apply(ctx) {
    ...
    const DEFAULT_PLUGIN_VERSION = '0.0.9';
    ...
    // 全局内存配置缓存与订阅机制（完全脱离 localStorage，直接同步 ~/.dsh/translator-config.json）
        ...
        if (res.ok) {
          ...
          if (data.success && data.config) {
            storeState = {
              ...
              history: Array.isArray(data.config.history) ? data.config.history : []
            };
            storeLoaded = true;
    ...
    async function updateServerConfig(patch) {
      storeState = {
        ...
        history: patch.history !== undefined ? patch.history : storeState.history
      };
      notifyStoreListeners();
    ...
    function saveHistoryItem(original, translated, model) {
      ...
      updateServerConfig({ history: list });
    }

    function removeHistoryItem(id) {
      const list = (storeState.history || []).filter(it => it.id !== id);
      updateServerConfig({ history: list });
    }

    function clearAllHistory() {
      updateServerConfig({ history: [] });
    }
...
3. 绝对禁则：
   - 只输出翻译后的纯净结果，严禁输出任何问候、引言、多余解释或前后引号。`;
    ...
    function TranslatorModal({ onClose, initialText, sessionId, currentModelInfo, onConfirmReplace, onConfirmAppend }) {
      ...
      const historyList = fullConfig.history || [];
