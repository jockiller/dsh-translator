...
function readDiskConfig() {
  try {
    ...
    console.warn('[dsh-plugin-translator] read disk config failed:', e.message);
    return { ...DEFAULT_CONFIG };
  }
}
...
function writeDiskConfig(data) {
  try {
    ...
    console.error('[dsh-plugin-translator] write disk config failed:', e.message);
    throw e;
  }
}
...
export const name = 'dsh-plugin-translator';
export const inject = [];
...
export function apply(ctx) {
  try {
    ...
    ctx.inject(['settings'], (sctx) => {
      try {
        ...
        sctx.settings?.register?.('dsh-plugin-translator', mockSchema, { base: { enabled: true } });
      } catch (e) {
        console.warn('[dsh-plugin-translator] settings register warning:', e.message);
      }
    });
  } catch (e) {
    console.warn('[dsh-plugin-translator] settings service inject failed:', e.message);
  }
  ...
  try {
          ...
          if (pathname === '/api/translator/translate' && req.method === 'POST') {
            ...
            req.on('end', async () => {
              try {
                ...
                let targetModelName = (model && model.trim()) || (reqModel && reqModel.trim()) || '';
                if (targetEndpoint.includes('bigmodel') && (!targetModelName || targetModelName.includes('gemini') || targetModelName.includes('claude') || targetModelName.includes('deepseek') || targetModelName.includes('gpt'))) {
                  targetModelName = 'glm-4-flash';
                }
                if (!targetModelName) {
                  targetModelName = 'glm-4-flash';
                }
    ...
    }, 'dsh-plugin-translator: webServer routes');
      } catch (err) {
        console.error('[dsh-plugin-translator] register route error:', err.message);
      }
    });
  } catch (e) {
    console.error('[dsh-plugin-translator] webServer service inject failed:', e.message);
  }
}
