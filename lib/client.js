/* dsh-plugin-translator client bundle - v0.0.8 */
(function() {
  function initModule(require) {
    ...
    var plugin = (function() {
// Client-side Cordis Plugin for DSH Input Translator
...
return {
  ...
  apply(ctx) {
    try {
      ...
      if (!slots) {
        console.warn('[dsh-plugin-translator] slots service not available');
        return;
      }

    const DEFAULT_PLUGIN_VERSION = '0.0.8';
    const STORAGE_KEY_ENABLED = 'dsh_translator_enabled';
    const STORAGE_KEY_ACTIVE_PROVIDER = 'dsh_translator_active_provider';
    const STORAGE_KEY_PROVIDERS_STORE = 'dsh_translator_providers_v3';
    const STORAGE_KEY_TARGET_LANG = 'dsh_translator_target_lang';
    const STORAGE_KEY_HISTORY = 'dsh_translator_history_v1';

    // 遗留 key 兼容
    const STORAGE_KEY_LEGACY_KEY = 'dsh_translator_api_key';
    const STORAGE_KEY_LEGACY_USE_CURRENT = 'dsh_translator_use_current_model';
    const STORAGE_KEY_LEGACY_MODEL = 'dsh_translator_model';
    const STORAGE_KEY_LEGACY_ENDPOINT = 'dsh_translator_endpoint';
    ...
    function getFullConfig() {
      try {
        let activeProvider = localStorage.getItem(STORAGE_KEY_ACTIVE_PROVIDER);
        if (!activeProvider) {
          const legacyUseCurrent = localStorage.getItem(STORAGE_KEY_LEGACY_USE_CURRENT);
          activeProvider = legacyUseCurrent === 'false' ? 'zhipu' : 'current';
        }

        let store = {};
        const rawStore = localStorage.getItem(STORAGE_KEY_PROVIDERS_STORE);
        if (rawStore) {
          try { store = JSON.parse(rawStore) || {}; } catch (_) {}
        }

        const legacyKey = localStorage.getItem(STORAGE_KEY_LEGACY_KEY) || '';
        const legacyModel = localStorage.getItem(STORAGE_KEY_LEGACY_MODEL) || '';
        const legacyEndpoint = localStorage.getItem(STORAGE_KEY_LEGACY_ENDPOINT) || '';

        const zhipu = {
          ...BUILTIN_PROVIDERS.zhipu,
          ...(store.zhipu || {})
        };
        if (!zhipu.apiKey && legacyKey) {
          zhipu.apiKey = legacyKey;
        }

        const current = {
          ...BUILTIN_PROVIDERS.current,
          ...(store.current || {})
        };

        // 自定义列表（新版数组结构）
        let customs = Array.isArray(store.customs) ? store.customs.filter(c => c && c.id) : [];

        // 旧版迁移：单个 custom 或 siliconflow 或遗留 key
        const legacySingle = store.custom || store.siliconflow;
        if (legacySingle && (legacySingle.endpoint || legacySingle.apiKey || legacySingle.model) && customs.length === 0) {
          customs = [makeCustomProvider(
            legacySingle.name || '自定义 API',
            legacySingle.endpoint || '',
            legacySingle.model || '',
            legacySingle.apiKey || ''
          )];
          if (legacyModel && legacyModel !== 'glm-4-flash' && !legacyModel.includes('Qwen') && !legacySingle.model) {
            customs[0].model = legacyModel;
          }
        } else if (legacyEndpoint && !legacyEndpoint.includes('bigmodel') && !legacyEndpoint.includes('siliconflow') && customs.length === 0) {
          customs = [makeCustomProvider('自定义 API', legacyEndpoint, legacyModel && legacyModel !== 'glm-4-flash' ? legacyModel : '', legacyKey)];
        }
        ...
        return {
          ...
          targetLang: localStorage.getItem(STORAGE_KEY_TARGET_LANG) || '智能中英互译',
          activeProvider,
          customs,
          zhipuOverride: zhipu.apiKey !== BUILTIN_PROVIDERS.zhipu.apiKey || zhipu.model !== BUILTIN_PROVIDERS.zhipu.model || zhipu.endpoint !== BUILTIN_PROVIDERS.zhipu.endpoint ? zhipu : undefined
        };
      } catch (e) {
        return {
          enabled: true,
    ...
    function getProviderById(cfg, provId) {
      if (provId === 'zhipu') return cfg.zhipuOverride || BUILTIN_PROVIDERS.zhipu;
      if (BUILTIN_PROVIDERS[provId]) return BUILTIN_PROVIDERS[provId];
    ...
    function saveFullConfig(cfg) {
      try {
        if (cfg.activeProvider) {
          localStorage.setItem(STORAGE_KEY_ACTIVE_PROVIDER, cfg.activeProvider);
        }
        if (cfg.enabled !== undefined) {
          localStorage.setItem(STORAGE_KEY_ENABLED, cfg.enabled ? 'true' : 'false');
        }
        if (cfg.targetLang) {
          localStorage.setItem(STORAGE_KEY_TARGET_LANG, cfg.targetLang);
        }
        if (cfg.customs !== undefined || cfg.providers !== undefined) {
          const store = {
            customs: cfg.customs || [],
            zhipu: (cfg.providers && cfg.providers.zhipu) || cfg.zhipuOverride || undefined
          };
          localStorage.setItem(STORAGE_KEY_PROVIDERS_STORE, JSON.stringify(store));
        }
      } catch (e) {}
    }
    ...
    function removeHistoryItem(id) {
      try {
        let history = getHistory().filter(it => it.id !== id);
        localStorage.setItem(STORAGE_KEY_HISTORY, JSON.stringify(history));
        return history;
      } catch (e) {
        return [];
...
3. 绝对禁则：
   - 只输出翻译后的纯净结果，严禁输出任何问候、引言、多余解释或前后引号。`;
    ...
    function TranslatorModal({ onClose, initialText, sessionId, currentModelInfo, onConfirmReplace, onConfirmAppend }) {
      ...
      const [fullConfig, setFullConfig] = React.useState(() => getFullConfig());
      ...
      const doTranslate = React.useCallback(async (textToTranslate, targetProvId) => {
        ...
        const provId = targetProvId || fullConfig.activeProvider || 'current';
        const prov = getProviderById(fullConfig, provId);
        if (!prov) return;
        ...
        try {
          const reqBody = {
            ...
            messages: buildMessages(text, fullConfig.targetLang),
            temperature: 0.1
          };
      ...
      }, [fullConfig, sessionId, currentModelInfo, activeModelLabel]);

      const handleSwitchProvider = (newId) => {
        if (!getProviderById(fullConfig, newId)) return;
        const next = { ...fullConfig, activeProvider: newId };
        setFullConfig(next);
        saveFullConfig(next);
      ...
      React.useEffect(() => {
        if (initialText && initialText.trim()) {
          ...
          // 如果输入框没有内容，默认直接切到历史记录 Tab 便于用户找回
          const h = getHistory();
          if (h.length > 0) setTab('history');
        }
      }, [initialText, doTranslate]);
    ...
    function TranslatorOverlayIcon(props) {
      ...
      const isEnabled = typeof localStorage !== 'undefined' ?
        localStorage.getItem(STORAGE_KEY_ENABLED) !== 'false' : true;
      ...
      return React.createElement(React.Fragment, null,
        ...
        // 弹窗
        modalOpen ? React.createElement(TranslatorModal, {
    ...
    function TranslatorSettingsCard() {
      ...
      const updateGeneral = (field, value) => {
        const next = { ...fullConfig, [field]: value };
        setFullConfig(next);
        saveFullConfig(next);
        triggerSaveNotice();
      };
      ...
      const handleSelectActive = (provId) => {
        ...
        const next = { ...fullConfig, activeProvider: provId };
        setFullConfig(next);
        saveFullConfig(next);
        triggerSaveNotice();
      };
      ...
      const updateProviderField = (provId, field, value) => {
        ...
        if (provId === 'zhipu') {
          const updated = { ...(fullConfig.zhipuOverride || BUILTIN_PROVIDERS.zhipu), [field]: value };
          const next = { ...fullConfig, zhipuOverride: updated };
          setFullConfig(next);
          saveFullConfig(next);
          triggerSaveNotice();
          return;
      ...
      const removeCustomProvider = (provId) => {
        ...
        if (selectedTab === provId) {
          setSelectedTab(next.activeProvider || 'current');
        }
        triggerSaveNotice();
      };

      const currentEditing = selectedTab === 'zhipu'
        ? (fullConfig.zhipuOverride || BUILTIN_PROVIDERS.zhipu)
    ...
    slots.inject('settings.plugin.item', () => slots.register(
      {
        ...
        key: 'dsh-plugin-translator',
        order: 50
      },
      TranslatorSettingsCard
    ));
  } catch (err) {
    console.error('[dsh-plugin-translator] client apply error:', err);
  }
  }
};
    ...
    exports.name = "dsh-plugin-translator";
