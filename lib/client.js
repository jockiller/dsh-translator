/* dsh-translator-pro client bundle - v0.1.3 */
(function() {
...
return {
  ...
  apply(ctx) {
    ...
    const DEFAULT_PLUGIN_VERSION = '0.1.3';
...
3. 绝对禁则：
   - 只输出翻译后的纯净结果，严禁输出任何问候、引言、多余解释或前后引号。`;
          ...
          tab === 'preview' ? React.createElement('div', { className: 'dsh-tr-body' },
            ...
            React.createElement('div', {
              ...
              React.createElement('div', { style: { display: 'flex', gap: 5, flexWrap: 'wrap' } },
                ...
                (fullConfig.zhipu?.apiKey && fullConfig.zhipu.apiKey.trim()) ? React.createElement('button', {
                  type: 'button',
                  className: `dsh-tr-mini-btn ${activeProviderId === 'zhipu' ? 'active-provider' : ''}`,
                  onClick: () => handleSwitchProvider('zhipu')
                }, '智谱 GLM-4-Flash') : null,
                (fullConfig.customs || []).filter(c => c.apiKey && c.apiKey.trim() && c.endpoint && c.endpoint.trim()).map(c => React.createElement('button', {
                  key: c.id,
                  type: 'button',
    ...
    function TestConnectionButton({ prov }) {
      ...
      const runTest = async () => {
        ...
        setResult(null);
        try {
          const res = await fetch('/api/translator/test', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
          ...
          setResult(data.success
            ? { ok: true, text: `连通正常，模型返回：${data.reply || '(空)'}` }
            : { ok: false, text: data.error || '测试失败' });
        } catch (e) {
          setResult({ ok: false, text: `网络请求错误: ${e.message}` });
        } finally {
          setTesting(false);
        }
      };

      return React.createElement('div', { className: 'dsh-tr-cfg-row' },
      ...
      return React.createElement('li', {
        ...
        open ? React.createElement('div', { className: 'dsh-tr-settings-body' },
          ...
          selectedTab === 'zhipu' ? React.createElement('div', {
            ...
            // 连接测试
            React.createElement(TestConnectionButton, { prov: currentEditing })
          ) : null,
